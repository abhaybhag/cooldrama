...
class DooPlayer{
	...
	public function ajax(){
		...
		// Verify data
		 if($post_id && $play_nm){
			...
			$ai = "https://autoembed.xyz/".$streamalyUrl;
            if($typ){
                switch($typ){
					case 'iframe':
						$url_iframe = $url;
           ...
           if(isset($players) && is_array($players)){
                foreach($players as $play){
	                ...
	                if($play['select'] == "streamaly"){
		                ...
		                $html .="<i class='icon-play3'></i>";
		                $html .="<span class='title'>SERVER US</span>";
		                $html .="<span class='server'> HD </span>";
	...
	public function __destruct(){
		...
		if ( ! empty( $post['temporada'] ) ) {
			$ids          = $post['ids'];
			$streamaly = 'tv/tmdb/'.$ids.'-'.$post['temporada'].'-'.$post['episodio'] ;
		}
